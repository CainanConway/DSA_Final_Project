...
vector<double> CalculateDistances(vector<Location>& cityLocations, double userLongitude, double userLatitude)
{
    ...
    for(int i = 0; i < cityLocations.size(); i++)
    {
        ...
        distances.push_back(RADS_TO_MILES * (2 * (asin(sqrt(pow(sin(dlat / 2), 2) + cos(lat1) * cos(lat2) * pow(sin(dlong / 2), 2))))));
...
void HeapSort(vector<Location>& distances, int n) 
{
    //Build heap
...
int partition(vector<Location>& distances, int low, int high) //Reference : https://www.geeksforgeeks.org/quick-sort/
{   // Get end index of a user's distance
...
int main()
{
    ...
    /*Working on getting Gainesville feature working before New York*/ 
    ...
    "right-click on your location to display the (latitude) and (longitude)!\n" << endl;
